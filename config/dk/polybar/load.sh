...
# wait a little for dk to load completely. ymmv.
sleep 1
...
if [ $? -ne 0 ]; then
  ...
  polybar -c $HOME/.config/dk/polybar/config.ini -q main &
fi
...
if [ $? -eq 0 ]; then
  ...
  polybar -c $HOME/.config/dk/polybar/config-ext.ini -q main-ext -r &
fi
