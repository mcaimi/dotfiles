#!/bin/bash
#
# QTile desktop library
# Helper functions
#

# load monitor information
source $HOME/.config/qtile/scripts/globals.sh
...
function key_load() {
 if [[ -e $SSH_KEYPATH/$1 ]]; then
  if lookup_key $1; then
    info "[$(date)]: Key $1 already registered in the keyring" $QTILELOG
    return 0
  else
    ssh-add $SSH_KEYPATH/$1
    info "[$(date)]: Added $1 to the ssh keyring" $QTILELOG
    return 0
  fi
 else
   info "[$(date)]: Key $1 not found." $QTILELOG
   return 1
 fi
}
...
# Call qtile-lock
function qtile_lock() {
  info "[$(date)]: dm-tool called explicitly by user" $QTILELOG
  dm-tool lock
}
...
function suspend() {
  MODE=$1
  if [[ "$1" == "suspend" ]]; then
    info "[$(date)]: SUSPEND-TO-RAM REQUESTED: initiating via systemd..." $QTILELOG
    systemctl suspend
  elif [[ "$1" == "hibernate" ]]; then
    info "[$(date)]: HIBERNATE ACTION REQUESTED: initiating via systemd..." $QTILELOG
    systemctl hibernate
  else
    error "[$DATE]: Unknown action requested. Execution aborted" $QTILELOG
    return -1
  fi
}
