...
function conky_draw_hud()
  ...
  -- draw center point
  conky_arc_draw(cr, 2, anchor_x, anchor_y, 0, 360, xrdb_colors.white, 4)
  -- draw HUD lines
  conky_draw_line(cr, anchor_x, anchor_y, (width - 40), anchor_y, 0.6, xrdb_colors.white, 1.0)
  conky_draw_line(cr, (width - 40), anchor_y, (width - 40), (anchor_y - 40), 0.6, xrdb_colors.white, 1.0)
  -- draw end point
  conky_arc_draw(cr, 2, (width - 40), (anchor_y - 40), 0, 360, xrdb_colors.white, 4)

  conky_arc_draw(cr, 20, anchor_x, anchor_y, 0.8, 360, xrdb_colors.white, 2.0)

  -- draw bells and whistles (CPU FREQUENCY)
  conky_draw_line(cr, anchor_x + 40, anchor_y + 40, anchor_x + cpu_frequency_ring.radius, 
      (anchor_y + cpu_frequency_ring.radius), 0.8, xrdb_colors.white, 1.5)

  conky_arc_draw(cr, 2, anchor_x + 40, anchor_y + 40, 0.8, 360, xrdb_colors.white, 4)
  conky_arc_draw(cr, 2, anchor_x + cpu_frequency_ring.radius, anchor_y + cpu_frequency_ring.radius, 0, 360, xrdb_colors.white, 4)
  ...
  conky_draw_line(cr, anchor_x + cpu_frequency_ring.radius, anchor_y + cpu_frequency_ring.radius - 15, 
      ...
      (anchor_y + cpu_frequency_ring.radius) + 80, 1.0, xrdb_colors.white, 2.0)
...
function conky_draw_filesystem_widget(filesystem)
  ...
  conky_text_write_custom(cr, width - 100, anchor_y + 45,
       "Filesystem", font_properties.size, xrdb_colors.white, 1.0)
end
